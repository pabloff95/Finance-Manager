...
function fillIncomeTable(){
    // Get categories
    $categories = getIncome(tableOrder());
    ...
    if (isset($_POST['edit'])){
        echo "<form action='management.php?data=income' method='POST' id='changeIncomeForm'> </form>";
    }
}
...
function recordFormatIncome($row){
    if (isset($_POST['edit']) && ($_POST['editId']) == $row['id']){
        ...
        echo "
              ...
              <td><input type='text' value='".$row['amount']."' name='inputAmount' form='changeIncomeForm' ></td>
              <td><input type='text' value='".$row['income']."' name='inputConcept' form='changeIncomeForm' ></td>
              <td>
                    <select name='inputCategory' form='changeIncomeForm'>";
                    printEditCategoriesIncome($row['category']);
              echo "<select>                        
              </td>
              <td><input type='date' value='".$row['date']."' name='inputDate' form='changeIncomeForm' ></td>
              <td><input type='submit' value='Change' name='changeIncome' form='changeIncomeForm' ></td>                       
        ...
        echo "<td>".$row['amount']."</td>
              ...
              <td>
                <form action='management.php?data=income' method='POST'>
                    <input type='submit' value='Delete' name='deleteIncome'>
